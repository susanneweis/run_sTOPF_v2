...
def main(base_path,proj,nn_mi,movies_properties):
    ...
    for mv_str in movies:
        ...
        title = f"Sex Score Clusters HDBSCAN {mv_str} corr"
        name_str = f"{mv_str}_HDBSCAN_clusters_corr"
        ...
        title = f"Sex Score Clusters HDBSCAN {mv_str} nn{nn_mi}"
        name_str = f"{mv_str}_HDBSCAN_clusters_nn{nn_mi}"
        create_glassbrains(cluster_assign_file, roi_fill_name, roi_value_name, roi_names, atlas_path, title, out_path, name_str)
