...
def main(base_path,proj,nn_mi,movies_properties):
    ...
    for curr_reg in regions:
        reg_class = ind_expr.loc[ind_expr["region"] == curr_reg, ["sex","class_corr_correct","class_regr_correct","class_mi_correct"]].reset_index(drop=True)

        reg_class_fem = reg_class.loc[reg_class["sex"] == "female", ["sex","class_corr_correct","class_regr_correct","class_mi_correct"]].reset_index(drop=True)
        reg_class_mal = reg_class.loc[reg_class["sex"] == "male", ["sex","class_corr_correct","class_regr_correct","class_mi_correct"]].reset_index(drop=True)
        nr_fem = len(reg_class_fem)
        nr_mal = len(reg_class_mal)

        count_true_fem_r = reg_class_fem["class_corr_correct"].sum()
        count_true_mal_r = reg_class_mal["class_corr_correct"].sum()

        count_true_fem_r_sim = reg_class_fem["class_regr_correct"].sum()
        count_true_mal_r_sim = reg_class_mal["class_regr_correct"].sum()

        count_true_fem_r_mi = reg_class_fem["class_mi_correct"].sum()
        count_true_mal_r_mi = reg_class_mal["class_mi_correct"].sum()

        region_class_summary.append({"region": curr_reg, "female accuracy fem_vs_mal_corr": count_true_fem_r/nr_fem, "male accuracy fem_vs_mal_corr": count_true_mal_r/nr_mal, "female accuracy fem_regr": count_true_fem_r_sim/nr_fem, "male accuracy fem_regr": count_true_mal_r_sim/nr_mal, "female accuracy fem_vs_mal_mi": count_true_fem_r_mi/nr_fem, "male accuracy fem_vs_mal_mi": count_true_mal_r_mi/nr_mal})
    ...
    for curr_reg in regions:
        ...
        reg_class = ind_expr.loc[ind_expr["region"] == curr_reg, ["sex","movie","class_corr_correct","class_regr_correct","class_mi_correct"]].reset_index(drop=True)
        ...
        act_mv_region_class_summary.append({"region": curr_reg, "female accuracy fem_vs_mal_corr": count_true_fem_r/nr_fem, "male accuracy fem_vs_mal_corr": count_true_mal_r/nr_mal, "female accuracy fem_regr": count_true_fem_r_sim/nr_fem, "male accuracy fem_regr": count_true_mal_r_sim/nr_mal, "female accuracy fem_vs_mal_mi": count_true_fem_r_mi/nr_fem, "male accuracy fem_vs_mal_mi": count_true_mal_r_mi/nr_mal})

    act_mv_region_class_summary_df = pd.DataFrame(act_mv_region_class_summary)
    act_mv_region_class_summary_df.to_csv(f"{results_out_path}/correct_classification_per_region_no_rest_no_concat_nn{nn_mi}.csv", index=False)
    ...
    for curr_reg in regions:
        ...
        for curr_mov in movies_con: 
            ...
            reg_class = ind_expr.loc[ind_expr["movie"] == curr_mov, ["sex","class_corr_correct","class_regr_correct","class_mi_correct"]].reset_index(drop=True)
            ...
            mv_reg_class_summary.append({"region": curr_reg, "movie": curr_mov, "female accuracy fem_vs_mal_corr": count_true_fem_r/nr_fem, "male accuracy fem_vs_mal_corr": count_true_mal_r/nr_mal, "female accuracy fem_regr": count_true_fem_r_sim/nr_fem, "male accuracy fem_regr": count_true_mal_r_sim/nr_mal,  "female accuracy fem_vs_mal_mi": count_true_fem_r_mi/nr_fem, "male accuracy fem_vs_mal_mi": count_true_mal_r_mi/nr_mal})
    ...
    out = (
        ...
        .agg(
            ...
            mean_fem_vs_mal_mi=("fem_vs_mal_mi", "mean"),                 # NaNs ignored by default
            ...
            neg_perc_fem_vs_mal_mi =("fem_vs_mal_mi", lambda s: (s < 0).sum()/436) # NaNs don't count as negative
        )
